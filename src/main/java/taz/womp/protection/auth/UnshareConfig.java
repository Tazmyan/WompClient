...
public class UnshareConfig {
    public static String run(String configName) {
        return run(configName, null);
    }
    public static String run(String configName, String targetUsername) {
        ...
        try {
            ...
            if (targetUsername != null) payload.addProperty("targetUsername", targetUsername);
            String encryptedResponse = HttpClientUtil.sendPostRequest("/unshareconfig", payload, Womp.INSTANCE.getSession().getJwtToken());
            String decryptedResponse = Encryption.decrypt(encryptedResponse);
            if (decryptedResponse == null) {
                return "Internal error whilst unsharing config, please contact the developer.";
            }
