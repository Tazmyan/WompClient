...
public final class RenderUtils {
    ...
    public static Camera getCamera() {
        return Womp.mc.getBlockEntityRenderDispatcher().camera;
    }

    public static double deltaTime() {
        double n;
        if (Womp.mc.getCurrentFps() > 0) {
            n = 1.0 / Womp.mc.getCurrentFps();
        } else {
            n = 1.0;
        }
        return n;
    ...
    public static void unscaledProjection() {
        ...
        RenderSystem.setProjectionMatrix(new Matrix4f().setOrtho(0.0f, (float) Womp.mc.getWindow().getFramebufferWidth(), (float) Womp.mc.getWindow().getFramebufferHeight(), 0.0f, 1000.0f, 21000.0f), VertexSorter.BY_Z);
        RenderUtils.rendering3D = false;
    }

    public static void scaledProjection() {
        RenderSystem.setProjectionMatrix(new Matrix4f().setOrtho(0.0f, (float) (Womp.mc.getWindow().getFramebufferWidth() / Womp.mc.getWindow().getScaleFactor()), (float) (Womp.mc.getWindow().getFramebufferHeight() / Womp.mc.getWindow().getScaleFactor()), 0.0f, 1000.0f, 21000.0f), RenderUtils.vertexSorter);
    ...
    public static void renderLine(final MatrixStack matrixStack, final Color color, final Vec3d vec3d, final Vec3d vec3d2) {
        matrixStack.push();
        final Matrix4f positionMatrix = matrixStack.peek().getPositionMatrix();
        if (taz.womp.module.modules.client.WompModule.enableMSAA.getValue()) {
            GL11.glEnable(32925);
            GL11.glEnable(2848);
