...
public final class WorldUtils {
    private static MinecraftClient getMc() { return Womp.mc; }

    public static boolean isDeadBodyNearby() {
        return getMc().world.getPlayers().parallelStream()
                .filter(e -> e != getMc().player)
                .filter(e -> e.squaredDistanceTo(getMc().player) <= 36)
    ...
    public static Entity findNearestEntity(PlayerEntity toPlayer, float radius, boolean seeOnly) {
        ...
        assert getMc().world != null;
        for (Entity e : getMc().world.getEntities()) {
            float d = e.distanceTo(toPlayer);

            if (e != toPlayer && d <= radius && getMc().player.canSee(e) == seeOnly) {
                if (d < mr) {
                    mr = d;
                    entity = e;
                }
    ...
    public static HitResult getHitResult(double radius) {
        return getHitResult(getMc().player, false, getMc().player.getYaw(), getMc().player.getPitch(), radius);
    }

    public static HitResult getHitResult(PlayerEntity entity, boolean ignoreInvisibles, float yaw, float pitch, double distance) {
        if (entity == null || getMc().world == null) return null;
        ...
        HitResult result = getMc().world.raycast(new RaycastContext(cameraPosVec, range, RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.NONE, entity));
    ...
    public static void placeBlock(BlockHitResult blockHit, boolean swingHand) {
        ActionResult result = getMc().interactionManager.interactBlock(getMc().player, Hand.MAIN_HAND, blockHit);
        if (result.isAccepted() && result.shouldSwingHand() && swingHand) getMc().player.swingHand(Hand.MAIN_HAND);
    }

    public static Stream<WorldChunk> getLoadedChunks() {
        int radius = Math.max(2, getMc().options.getClampedViewDistance()) + 3;
        int diameter = radius * 2 + 1;

        ChunkPos center = getMc().player.getChunkPos();
        ...
        return Stream.iterate(min, pos -> {
                ...
                .filter(c -> getMc().world.isChunkLoaded(c.x, c.z))
                .map(c -> getMc().world.getChunk(c.x, c.z)).filter(Objects::nonNull);
    }

    public static boolean isShieldFacingAway(PlayerEntity player) {
        if (getMc().player != null && player != null) {
            Vec3d playerPos = getMc().player.getPos();
