...
public final class Utils {
    public static Color getMainColor(final int n, final int n2) {
        final int f = WompModule.redColor.getIntValue();
        final int f2 = WompModule.greenColor.getIntValue();
        final int f3 = WompModule.blueColor.getIntValue();
        if (WompModule.enableRainbowEffect.getValue()) {
            return ColorUtil.a(n2, n);
        }
        if (WompModule.enableBreathingEffect.getValue()) {
            return ColorUtil.alphaStep_NumberOne(new Color(f, f2, f3, n), n2, 20);
        }
